...
class DirectChatRoomMemberSequenceRepositoryImpl(
    ...
    override fun updateReadSequence(chatRoomId: ChatRoomId, userId: UserId, chatLogSequence: ChatLogSequence) {
        val query = Query(
    ...
    override fun updateStartSequence(chatRoomId: ChatRoomId, userId: UserId, chatLogSequence: ChatLogSequence) {
        val query = Query(
            Criteria.where("chatRoomId").`is`(chatRoomId.id)
                .and("memberId").`is`(userId.id)
        )
